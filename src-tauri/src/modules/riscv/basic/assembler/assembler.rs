use crate::interface::assembler::{Assembler, AssembleResult, AssemblyError, Instruction, InstructionSet, Memory, Operand};
use crate::interface::parser::{ParserResult, ParserInst};
...
impl Assembler<RISCV> for RiscVAssembler {
    fn assemble(
        ...
        for (index, element) in ast.text.iter().enumerate() {
            ...
            let opd: Vec<ParserRISCVInstOpd> = new_instruction.operands.iter()
                .map(|operand| match operand {
                    Operand::Reg(reg) => ParserRISCVInstOpd::Reg(reg.clone()),
                    Operand::Operator(imm) => ParserRISCVInstOpd::Imm(ParserRISCVImmediate::Imm(imm.clone())),
                })
                .collect();
...
fn process_code(index: usize, element: &ParserResultText<RISCV>) -> Result<u32, Vec<AssemblyError>> {
    let mut line: u32 = 0;
    let mut error: Vec<AssemblyError> = Vec::new();
    match element {
        ParserResultText::Text(inst) => {
