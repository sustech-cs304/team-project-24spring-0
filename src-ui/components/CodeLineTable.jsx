    ...
    if (selectedKeys === 'all') {
      ...
      for (let line in lines) {
        if (!(line in file.selectedLines)) {
          const result = await invoke('set_breakpoint', {
            line: parseInt(line),
          });
      ...
      if (selectedKeys.anchorKey === undefined) {
        ...
        for (let line in lines) {
          const result = await invoke('remove_breakpoint', {
            line: parseInt(line),
          });
          outputStore.addOutput('Breakpoint removed at line ' + line);
        }
      } else if (
        selectedLines.find(key => key === selectedKeys.anchorKey.toString())
      ) {
        const result = await invoke('set_breakpoint', {
          line: parseInt(selectedKeys.anchorKey),
        });
        outputStore.addOutput(
          'Breakpoint set at line ' + selectedKeys.anchorKey,
        );
      } else {
        const result = await invoke('remove_breakpoint', {
          line: parseInt(selectedKeys.anchorKey),
        ...
        outputStore.addOutput(
          'Breakpoint removed at line ' + selectedKeys.anchorKey,
        );
      }
    }
  };
        ...
        <TableBody>
          {lines.map((line, index) => (
            <TableRow key={index}>
              ...
              <TableCell>
                {line.line + 1 + ': ' + codeLines[line.line]}
              </TableCell>
